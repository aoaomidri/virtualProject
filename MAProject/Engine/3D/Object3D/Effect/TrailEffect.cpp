...
void TrailEffect::Update(const EulerTransform& transform, const ViewProjection& viewProjection) {
...
void TrailEffect::DrawImgui(const std::string& imguiTag){
#ifdef _DEBUG
...
Microsoft::WRL::ComPtr<ID3D12Resource> TrailEffect::CreateBufferResource(size_t sizeInBytes) {
...
Model::ModelData TrailEffect::MakePrimitive() {
...
void TrailEffect::PositionReset(){
...
TrailEffect::Particle TrailEffect::MakeNewParticle(const Vector3& transform){
	Particle particle{};
	color_ = random_->DistributionV3(0.0f, 1.0f);
...
std::list<TrailEffect::Particle> TrailEffect::Emission(const Emitter& emitter){
...
bool TrailEffect::IsCollision(const AABB& aabb, const Vector3& point){
	//最近接点を求める
