...
void InstancingObject3D::Initialize(const std::string fileName) {
	makeResource();
...
void InstancingObject3D::Update(const ViewProjection& viewProjection) {
	if (!isDraw_) {
		return;
	}
...
void InstancingObject3D::DrawImgui(std::string name) {
#ifdef _DEBUG
...
void InstancingObject3D::SetDirectionalLight(const DirectionalLight::DirectionalLightData* light) {
	directionalLight_ = light;
}

void InstancingObject3D::SetPointLight(const PointLight::PointLightData* pLight) {
	pointLight_ = pLight;
}

Microsoft::WRL::ComPtr<ID3D12Resource> InstancingObject3D::CreateBufferResource(size_t sizeInBytes) {
...
void InstancingObject3D::makeResource() {
	//マテリアル用のリソース
	...
	//wvpInstancingResource_ = CreateBufferResource(sizeof(TransformationMatrix) * );
	//書き込むためのアドレスを取得
...
Vector3 InstancingObject3D::CalculateValue(const std::vector<Model::KeyframeVector3>& keyframes, float time) {
...
Quaternion InstancingObject3D::CalculateValue(const std::vector<Model::KeyframeQuaternion>& keyframes, float time) {
	assert(!keyframes.empty());
	if (keyframes.size() == 1 || time <= keyframes[0].time) {
		return keyframes[0].value;
	}
