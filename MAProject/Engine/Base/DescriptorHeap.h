...
class UAVDescriptorHeap {
public:
	UAVDescriptorHeap() = default;
	~UAVDescriptorHeap() = default;
	UAVDescriptorHeap(const UAVDescriptorHeap& input) = delete;
	UAVDescriptorHeap& operator=(const UAVDescriptorHeap&) = delete;

public:
	//namespace省略
	template <class T> using ComPtr = Microsoft::WRL::ComPtr<T>;

	static UAVDescriptorHeap* GetInstance();
...
public:
	...
	ID3D12DescriptorHeap* GetSRVHeap() const { return uavDescriptorHeap_.Get(); }


	D3D12_CPU_DESCRIPTOR_HANDLE GetCPUDescriptorHandle() {
		D3D12_CPU_DESCRIPTOR_HANDLE handleCPU = uavDescriptorHeap_->GetCPUDescriptorHandleForHeapStart();
		handleCPU.ptr += (descriptorSizeSRV_ * setHeapPositionCPU_);
		setHeapPositionCPU_++;
		return handleCPU;
	}
	...
	D3D12_GPU_DESCRIPTOR_HANDLE GetGPUDescriptorHandle() {
		D3D12_GPU_DESCRIPTOR_HANDLE handleGPU = uavDescriptorHeap_->GetGPUDescriptorHandleForHeapStart();
		handleGPU.ptr += (descriptorSizeSRV_ * setHeapPositionGPU_);
		setHeapPositionGPU_++;
		return handleGPU;
	}
...
private:
	//SRV
	ComPtr<ID3D12DescriptorHeap> uavDescriptorHeap_;

	const uint32_t kMaxHeapSize_ = 256;
	...
	const D3D12_DESCRIPTOR_HEAP_TYPE heapType_ = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
