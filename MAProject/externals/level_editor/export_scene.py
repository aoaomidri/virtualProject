...
class MYADDON_OT_export_scene(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    def export(self):
        ...
        with open(self.filepath, "wt") as file:
            ...
            for object in bpy.context.scene.objects:
                ...
                #シーン直下のオブジェクトをルートノードとして(深さ0)とし、再起関数で走査
    ...
    def parse_scene_recursive(self, file, object, level):
        ...
        self.write_and_print(file ,indent + "Rotate(%f,%f,%f)" % (rot.x, rot.y, rot.z) )
        self.write_and_print(file ,indent + "Scale(%f,%f,%f)" % (scale.x, scale.y, scale.z) )
        #カスタムプロパティ'file_name'
        if "file_name" in object:
            self.write_and_print(file, indent + "N %s" % object["file_name"])

        if "animation_name" in object:
            self.write_and_print(file, indent + "N %s" % object["animation_name"])

        if "disabled_option" in object:
            self.write_and_print(file, indent + "N %s" % object["disabled_option"])
    ...
    def export_json(self):
        ...
        for object in bpy.context.scene.objects:
            ...
            self.parse_scene_recursive_json(json_object_root["object"],object,0)
    ...
    def parse_scene_recursive_json(self, data_parent, object, level):
        ...
        if "animation_name" in object:
            json_object["animation_name"] = object["animation_name"]

        if "disabled_option" in object:
            json_object["disabled_option"] = object["disabled_option"]
